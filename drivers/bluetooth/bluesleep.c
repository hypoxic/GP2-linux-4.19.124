 ...
 *  2020-Mar-03  GoPro           Customize for GoPro design
 */
...
- Keep BT_DEV_WAKE high the whole time (from RTOS) to prevent the BT chip from going to sleep.
It will consume a little more power, but it's okay to do so when the camera is active.
- This driver is still needed to expose procfs nodes to userspace for BSA server.
However, all the reques to drive GPIOS will be ignore!
**Optimization Level 1**
- Let BSA actually drive BT_DEV_WAKE and let the BT chip sleep intermittently.
- Need more sophisticated driver
*/
#define GP_SUPPORT_UART_OPT	0	/* GoPro has no UART optimization! No on/off on the fly */
#define GP_SUPPORT_HOST_WAKE	0	/* BT_HOST_WAKE not even connected to SoC */

#define PWR_OPT_LVL_0	0
#define PWR_OPT_LVL_1	1
static int pwr_opt_lvl = PWR_OPT_LVL_0;
module_param_named(pwr_opt_lvl, pwr_opt_lvl, int, 0);
...
#else // GoPro!
static void bluesleep_rx_timer_expire(struct timer_list *unused);
static DEFINE_TIMER(rx_timer, bluesleep_rx_timer_expire);
#endif
...
/* GoPro porting implementation here */
/* Since GoPro has no UART power opt, we just fake it here 
It's possible to do one more round of cleanup to get rid of all redundant stuff
*/
static unsigned int pl011_tx_empty(struct uart_port *uport)
{
	return 1;
}
...
static struct uart_port *pl011_get_uart_port(int port_index)
{
	return NULL;
}
...
{
	/* GoPro HOST should not go to sleep */
	if (pwr_opt_lvl == PWR_OPT_LVL_0) {
		return 0;
	}
...
static int bluesleep_populate_dt_pinfo(struct platform_device *pdev)
{
	...
	tmp = of_get_named_gpio(np, "brcm,bt_host_wake", 0);
	if (tmp < 0) {
	...
	tmp = of_get_named_gpio(np, "brcm,bt_ext_wake", 0);
	if (tmp < 0)
		bsi->has_ext_wake = 0;
	else
		bsi->has_ext_wake = 1;
	...
	size_t count, loff_t *pos)
{
	...
	switch ((long)data) {
	case PROC_LPM:
		pr_err("bluesleep_proc_write PROC_LPM");
		if (lbuf[0] == '0') {
			/* HCI_DEV_UNREG */
			bluesleep_stop();
			...
			if (!has_lpm_enabled) {
				...
				bsi->uport = pl011_get_uart_port(BT_PORT_ID);
		...
		break;
	case PROC_BTWRITE:
		pr_err("bluesleep_proc_write PROC_BTWRITE");

		if (pwr_opt_lvl == PWR_OPT_LVL_0) {
			pr_err("Level 1: all request ignored!");
			break;
		}
...
#if GP_SUPPORT_UART_OPT /* the whole point of rx_timer is to optimize uart power */
	/* Initialize timer */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0)
	init_timer(&rx_timer);

...
#define MTP_BULK_BUFFER_SIZE       (4 << 20)   /* 4MB */
...
mtp_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	/* support super speed or super speed plus hardware */
	...
	DBG(cdev, "%s speed %s: IN/%s, OUT/%s\n",
		gadget_is_superspeed_plus(c->cdev->gadget) ? "super_plus" :
...
struct usb_function_instance *alloc_inst_mtp_ptp(bool mtp_config)
{
	...
	mtp_group = usb_os_desc_prepare_interf_dir(&fi_mtp->func_inst.group, 1,
						   descs, names, THIS_MODULE);
	if (IS_ERR(mtp_group)) {
		mtp_free_inst(&fi_mtp->func_inst);
		return ERR_CAST(mtp_group);
	}

...
int wifi_platform_set_power(wifi_adapter_info_t *adapter, bool on, unsigned long msec)
{
...
#ifdef DHD_REGON_PIN_CONTROL
#ifndef CONFIG_DTS
	struct wifi_platform_data *plat_data;
#endif
#endif /* DHD_REGON_PIN_CONTROL */
	...
	DHD_ERROR(("%s: wifi_platform_set_power, on=%d\n", __FUNCTION__, on));
	if (on) {
...
#else
if (on) {
	DHD_ERROR(("======== Delay %lu(ms) after WL_REGON pull-up ========\n", msec));
	if (msec)
		OSL_SLEEP(msec);
	is_power_on = TRUE;
}
else {
	DHD_ERROR(("======== It's OK to pull down WL_REGON now ========\n"));
	is_power_on = FALSE;
}
#endif /* DHD_REGON_PIN_CONTROL */

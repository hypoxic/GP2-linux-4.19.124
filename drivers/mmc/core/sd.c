...
static int __sd_select_driver_type(struct mmc_card *card, int card_drv_type,
				   u8 *status)
{
...
{
	return __sd_select_driver_type(card,
			card->sw_caps.sd3_drv_type | SD_DRIVER_TYPE_B,
			status);
}

static int sd_select_driver_type_33V(struct mmc_card *card, u8 *status)
{
	return __sd_select_driver_type(card, SD_DRIVER_TYPE_A, status);
}
...
retry:
	...
	if (rocr & SD_ROCR_S18A && mmc_host_uhs(host)) {
		...
		/* Set the driver strength for the card */
		err = sd_select_driver_type_33V(card, status);
		if (err)
			goto free_card;
	}

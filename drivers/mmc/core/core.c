...
static int mmc_mrq_prep(struct mmc_host *host, struct mmc_request *mrq)
{
    ...
    if (mrq->cmd->uhs2_cmd) {
        pr_debug("%s: starting UHS-II header %08x arg %08x\n",
            mmc_hostname(host), mrq->cmd->uhs2_cmd->header,
            mrq->cmd->uhs2_cmd->arg);
    }
	if (mrq->cmd) {
		mrq->cmd->error = 0;
		mrq->cmd->mrq = mrq;
	...
	if (host->flags & MMC_UHS2_SUPPORT &&
		...
		if (mrq->cmd->uhs2_cmd == NULL) {
			if (uhs2_prepare_sd_cmd(host, mrq))
				return 0;
		}
...
void mmc_power_off(struct mmc_host *host)
{
	...
	host->ios.clock = 0;
	host->ios.vdd = 0;
	host->ios.vdd2 = ((unsigned short)-1);
...
void mmc_rescan(struct work_struct *work)
{
	...
	if (mmc_card_is_removable(host) && host->ops->get_cd &&
		...
		if (host->ops->uhs1_to_uhs2 && host->ops->uhs1_to_uhs2(host))
			pr_err("%s: %s: Error change uhs1 to uhs2\n",
				mmc_hostname(host), __func__);
	...
	/*
	 * Before trying another 'f_init' we need
	 * to clear the power-state as well
	 */
	mmc_power_off(host);
	...
	if (host->caps & MMC_CAP_UHS2) {
		...
		if (host->ops->uhs2_to_uhs1 && host->ops->uhs2_to_uhs1(host))
			pr_err("%s: %s: Error change uhs2 to uhs1\n",
				mmc_hostname(host), __func__);
	}

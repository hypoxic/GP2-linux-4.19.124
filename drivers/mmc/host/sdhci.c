...
void sdhci_finish_data(struct sdhci_host *host)
{
	...
	/* Shall we send CMD12 or ABORT packet to device for error case?
	 */
	if (host->mmc->flags & MMC_UHS2_SUPPORT) {
...
static void sdhci_finish_command(struct sdhci_host *host)
{
	...
	if (host->mmc->flags & MMC_UHS2_SUPPORT)
		sdhci_uhs2_finish_command(host, cmd);
...
bool sdhci_wait_clock_stable(struct sdhci_host *host)
{
	ktime_t timeout;
	u16 clk;
...
EXPORT_SYMBOL_GPL(sdhci_wait_clock_stable);

void sdhci_enable_clk(struct sdhci_host *host, u16 clk)
{
    int ret;

	if (host->ops->clock_change_quirk) {
		ret = host->ops->clock_change_quirk(host);
		if (ret) {
			pr_err("%s: clock change quirk failed %d\n",
	...
	if (host->ops->clock_change_quirk2) {
		ret = host->ops->clock_change_quirk2(host);
		if (ret) {
			pr_err("%s: clock change quirk2 failed %d\n",
				mmc_hostname(host->mmc), ret);
			return;
		}
	}
	/* We do not set SDHCI_PLL_EN because the SDHCI_PLL_EN
		is not using */
	clk |= SDHCI_CLOCK_CARD_EN;
	sdhci_writew(host, clk, SDHCI_CLOCK_CONTROL);
	/* It need to wait 600us after Enable SD clockBooting */
	udelay(600);
}
EXPORT_SYMBOL_GPL(sdhci_enable_clk);
void sdhci_uhs2_set_clk(struct mmc_host *mmc)
{
...
void sdhci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->power_mode == MMC_POWER_OFF) {
		if (host->quirks2 & SDHCI_QUIRK2_OPS_SD_POWER_CONTROL)
			host->ops->set_power(host, ios->power_mode, ios->vdd, -1);
			//host->ops->sd_power_off(host);
	...
	if (ios->power_mode == MMC_POWER_UP) {
		if (host->quirks2 & SDHCI_QUIRK2_OPS_SD_POWER_CONTROL)
			host->ops->set_power(host, ios->power_mode,
				ios->vdd, ios->vdd2);
			//host->ops->sd_power_on(host);
	}
	...
	if (host->version >= SDHCI_SPEC_400) {
		...
		ctrl_2 = sdhci_readw(host, SDHCI_HOST_CONTROL2);
		ctrl_2 |= SDHCI_CTRL_V4_MODE;
		if (host->flags & SDHCI_USE_64_BIT_DMA)
			ctrl_2 |= SDHCI_CTRL_64BIT_ADDR;
		sdhci_writew(host, ctrl_2, SDHCI_HOST_CONTROL2);
...
static void sdhci_select_tuning_phase(struct sdhci_host *host)
{
	if (host->ops->select_tuning_quirk)
		host->ops->select_tuning_quirk(host);
}
...
int sdhci_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	case MMC_TIMING_UHS_SDR50:
		if ((host->flags & SDHCI_SDR50_NEEDS_TUNING) &&
		  !(host->quirks2 & SDHCI_QUIRK2_BROKEN_SDR50_TUNING))
			break;
...
static const struct mmc_host_ops sdhci_ops = {
	...
	.uhs2_to_uhs1		= sdhci_uhs2_to_uhs1,
	.uhs1_to_uhs2		= sdhci_uhs1_to_uhs2,
	.uhs2_set_clk		= sdhci_uhs2_set_clk,
};
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (intmask & SDHCI_INT_DATA_TIMEOUT) {
		host->data->error = -ETIMEDOUT;
		sdhci_select_tuning_phase(host);
	}
...
void __sdhci_read_caps(struct sdhci_host *host, u16 *ver, u32 *caps, u32 *caps1)
{
	...
	if (host->ops->set_version_quirk)
		v = host->ops->set_version_quirk(host);
	else
	...
	if (host->ops->read_caps) {
		host->ops->read_caps(host);
		if (caps)
			host->caps = *caps;
		if (caps1)
			host->caps1 = *caps1;
...
int sdhci_setup_host(struct sdhci_host *host)
{
    ...
    if (host->version >= SDHCI_SPEC_400) {
        if (host->caps1 & SDHCI_SUPPORT_UHS2) {
	...
	if (!mmc->f_max)
		mmc->f_max = max_clk;
	else if ((host->version < SDHCI_SPEC_400)
			&& (mmc->f_max > max_clk))
	/* because it can over 255MHZ at UHS2 */
		mmc->f_max = max_clk;
...
unirq:
	/* Shall I reset UHS2 and clear UHS2 irq here? */
...
int sdhci_uhs2_to_uhs1(struct mmc_host *mmc)
{
	int err = 0;
	struct sdhci_host *host = mmc_priv(mmc);

	if (host->ops->uhs2_to_uhs1)
		err = host->ops->uhs2_to_uhs1(host, sdhci_irq, sdhci_thread_irq);

	return err;
}

int sdhci_uhs1_to_uhs2(struct mmc_host *mmc)
{
	int err = 0;
	struct sdhci_host *host = mmc_priv(mmc);

	if (host->ops->uhs1_to_uhs2)
		err = host->ops->uhs1_to_uhs2(host, sdhci_irq, sdhci_thread_irq);
